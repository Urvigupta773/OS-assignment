...
sem_t project_semaphors[3];


char* types[3] = { "pen, paper , question paper" };


bool items_on_table[3] = { false, false, false };


ype item, manage these with this semaphore
...
{
int pen_id = *(int*) arg;
int type_id = pen_id % 3;


for (int i = 0; i < 3; ++i)
{
printf("pen %d Waiting for %s\n",pen_id, types[type_id]);


sem_wait(&project_semaphors[type_id]);


printf("\pen %d Now making the a cigarette\n", pen_id);
usleep(rand() % 50000);
sem_post(&agent_ready);


printf("pen %d Now smoking\n", pen_id);
usleep(rand() % 50000);
}
...
sem_post(&project_semaphors[(pusher_id + 2) % 3]);
}
else if (items_on_table[(pusher_id + 2) % 3])
{
items_on_table[(pusher_id + 2) % 3] = false;
sem_post(&project_semaphors[(pusher_id + 1) % 3]);
}
else
{
...
// Initialize the semaphores for the pens and pusher
for (int i = 0; i < 3; ++i)
{
sem_init(&project_semaphors[i], 0, 0);
...
if (pthread_create(&pen_threads[i], NULL, pen, &pen_ids[i]) == EAGAIN)
{
...
for (int i = 0; i < 6; ++i)
{
pthread_join(pen_threads[i], NULL);
}
